$(document).ready(function () {
            ...
            if (dateprop.data.content.length > 0) {
                if (dateprop.data.startTime.length > 0) {
                    ...
                    for (var i = 0; i < dateprop.data.startTime.length; i++) {
                        if (dateprop.data.allDay[i])
                            allDay(dateprop.data.content[i]);
                        else
                            date(dateprop.data.content[i], dateprop.data.startTime[i], dateprop.data.endTime[i]);
                    }
                }
            }
        });
    ...
    function init() {
        ...
        $('ul > li:not(.all-day)').each(function (i, e) {
            ...
            if (hour < 10)
                $(e).find('time > span').html('0' + hour + ':' + '00<h6>AM</h6>');
            else if (hour < 12 && hour >= 10)
                $(e).find('time > span').html(hour + ':' + '00<h6>AM</h6>');
            else if (hour == 12)
                $(e).find('time > span').html(hour + ':' + '00<h6>PM</h6>');
            else if (hour < 22 && hour > 12)
                $(e).find('time > span').html('0' + (hour - 12) + ':' + '00<h6>PM</h6>');
            else if (hour >= 22)
                $(e).find('time > span').html((hour - 12) + ':' + '00<h6>PM</h6>');
        });
    }
